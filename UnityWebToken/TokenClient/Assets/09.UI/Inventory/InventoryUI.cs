...
public class InventoryUI : WindowUI
{
    ...
    public override void Open()
    {
        ...
        //foreach(Item item in _itemDictionary.Values)
        //{
        //    Slot slot = FindSlotByNumber(item.slotNumber);
        //    slot.Child.RemoveFromHierarchy();
        //}
        ...
        NetworkManager.Instance.GetRequest("inven", "", (type, msg) =>
        {
            ...
            if(type == MessageType.SUCCESS) {
                InventoryVO invenVO = JsonUtility.FromJson<InventoryVO>(msg);
                invenVO.list.ForEach(itemVO =>
                {
                    ItemSO data = UIController.Instance.ItemList.Find(x => x.itemCode == itemVO.itemCode);
                    AddItem(data, itemVO.count, itemVO.slotNumber);
                });
            }
